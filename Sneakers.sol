...
contract ERC1155 is ERC165, IERC1155, IERC1155MetadataURI {
    ...
    string private uriLink = "https://theredapefamily.com/Gravity/data/Special";
    ...
    function setUri(string calldata UriLink) external manager {
       uriLink = UriLink;
    }

    function giveawayMint(address[] calldata winners, uint256[] calldata amount, uint256[] calldata id) external manager {
        
        uint256 length = amount.length;
        for(uint256 t; t < length; ++t) {
            
        _balances[id[t]][winners[t]] += amount[t];
        
        emit TransferSingle(msg.sender, address(0), winners[t], id[t], amount[t]);
        }
    }
    ...
    function mint() external {
        uint256 amount = TRAF.balanceOf(msg.sender, 0) / 2;
        amount += (TRAF.balanceOf(msg.sender, 1) / 2);
